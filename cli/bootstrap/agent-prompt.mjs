...
export function formatAgentSetupPrompt(plan, { setupUrl } = {}) {
  ...
  return [
    ...
    setupUrl
      ? `3. If I explicitly ask you to complete guided setup, open the private onboarding URL ${setupUrl} in the in-app browser, then walk me through creating the root user, signing in, and creating provisioner credentials. This URL is supplied in the private setup instruction; do not repeat it or its token elsewhere in chat or logs.`
      : "3. If I explicitly ask you to complete guided setup, derive the private onboarding URL locally from .tama.env and open it in the in-app browser without printing its setup token. Walk me through creating the root user, signing in, and creating provisioner credentials. If browser control is unavailable, direct me to tama/README.md without reproducing the token.",
