...
/**
 * Reads the configured public Tama port from an existing `tama/.tama.env`.
...
export function configuredPort(root) {
  try {
    const values = readEnvironmentValues(root, BOOTSTRAP_PATHS.environment);
 ...
 *   without `tama/.tama.env`, derived from the accepted MCP App contract.
 * @returns {number}
 */
export function resolveEnvironmentPort(root, requestedPort, freshDefaultPort) {
  const filename = join(root, BOOTSTRAP_PATHS.environment);
  ...
  if (!Number.isInteger(existingPort) || existingPort < 1 || existingPort > 65_535) {
    throw ownershipError(`${BOOTSTRAP_PATHS.environment} has an invalid TAMA_PORT: ${raw}`);
  }
...
function postgresEnvironment(values, filename) {
  ...
  for (const name of required) {
    if (!values.get(name)) {
      throw new Error(`${BOOTSTRAP_PATHS.environment} must define a non-empty ${name}`);
    }
  }
  return [
    `# Generated by Tama Kit from ${BOOTSTRAP_PATHS.environment}. Keep this file private.`,
    ...required.map(
  ...
  if (!existsSync(filename)) {
    ...
    return {
      ...
      postgresOperation: operationForContent(
        join(root, BOOTSTRAP_PATHS.postgresEnvironment),
