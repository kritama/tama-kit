...
export function planMcpAppAddition(
  ...
  const operations: FileOperation[] = [
    ignore,
    owned.plan(
      join(root, MCP_ADDITION.environment),
      `# Generated by Tama Kit. Project-owned; keep private.\n${Object.entries(variables)
  ...
  await writeScaffold(
    ...
    () => {
      ...
      for (const [name, value] of Object.entries(variables)) {
        ...
        // Resumed fragments belong to the developer; validate their semantics in the current reader.
        if (!options.resumeId && actual.runtime.environment.get(name) !== expected)
